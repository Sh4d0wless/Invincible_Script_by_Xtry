...
on *:dialog:gamez:init:0: {
  ...
  dll dll/mdx.dll SetFont 1,2,5 13 14 Tahoma 
  dll dll/mdx.dll SetBorderStyle $dname 1,5 clientedge
...
on *:dialog:gamez:sclick:5:{
  if ($did == 5) {
    ...
    if (%gamex == Chessmaster v1.309) { run utilities/games/satranc.exe }
    if (%gamex == Go-Moku v1.0) { run utilities/games/moku.exe }
    if (%gamex == Memory Blocks v1.00) { run utilities/games/blocks.exe }
    if (%gamex == Tetrix 99 v1.2) { run utilities/games/tetris.exe }
    if (%gamex == Hangman Puzzle v1.0) { run utilities/games/hangman.exe }
    if (%gamex == Lovex) { run utilities/games/lovex.exe }
    if (%gamex == Snake) { snake }
    if (%gamex == The Gun) { run utilities/games/the_gun.exe }
    if (%gamex == Slam! v1.0) { run utilities/games/slam.exe }
    if (%gamex == Uno v1.4) { run utilities/games/Uno.exe }
    if (%gamex == Taipei v4.00) { run utilities/games/tp.exe }
    if (%gamex == Pipe Mania) { run utilities/games/mania.exe }
    if (%gamex == Patience Pack v6.0) { run utilities/games/patpack.exe }
    if (%gamex == Pick Up Sticks v1.00) { run utilities/games/sticks.exe }
    if (%gamex == Pegged v1.0) { run utilities/games/pegged.exe }
    if (%gamex == 3-D Tic Tac Toe v1.0) { run utilities/games/tictac.exe }
    if (%gamex == JezzBall 1992) { run utilities/games/jezzball.exe }
    if (%gamex == Tamagotchi Simulator 2.0) { run utilities/games/tamagotchi/tamsim.exe }
  }
}
on *:dialog:gamez:dclick:2:{
  if ($did == 2) {
    set %gamex $did($dname,2).seltext
    if (%gamex == Chessmaster v1.309) { run utilities/games/satranc.exe | dialog -c gamez gamez }
    if (%gamex == Go-Moku v1.0) { run utilities/games/moku.exe | dialog -c gamez gamez }
    if (%gamex == Memory Blocks v1.00) { run utilities/games/blocks.exe | dialog -c gamez gamez }
    if (%gamex == Tetrix 99 v1.2) { run utilities/games/tetris.exe | dialog -c gamez gamez }
    if (%gamex == Hangman Puzzle v1.0) { run utilities/games/hangman.exe | dialog -c gamez gamez }
    if (%gamex == Lovex) { run utilities/games/lovex.exe | dialog -c gamez gamez }
    if (%gamex == Snake) { snake }
    if (%gamex == The Gun) { run utilities/games/the_gun.exe | dialog -c gamez gamez }
    if (%gamex == Slam! v1.0) { run utilities/games/slam.exe | dialog -c gamez gamez }
    if (%gamex == Uno v1.4) { run utilities/games/Uno.exe | dialog -c gamez gamez }
    if (%gamex == Taipei v4.00) { run utilities/games/tp.exe | dialog -c gamez gamez }
    if (%gamex == Pipe Mania) { run utilities/games/mania.exe | dialog -c gamez gamez }
    if (%gamex == Patience Pack v6.0) { run utilities/games/patpack.exe | dialog -c gamez gamez }
    if (%gamex == Pick Up Sticks v1.00) { run utilities/games/sticks.exe | dialog -c gamez gamez }
    if (%gamex == Pegged v1.0) { run utilities/games/pegged.exe | dialog -c gamez gamez }
    if (%gamex == 3-D Tic Tac Toe v1.0) { run utilities/games/tictac.exe | dialog -c gamez gamez }
    if (%gamex == JezzBall 1992) { run utilities/games/jezzball.exe | dialog -c gamez gamez }
    if (%gamex == Tamagotchi Simulator 2.0) { run utilities/games/tamagotchi/tamsim.exe | dialog -c gamez gamez }
  }
}
...
on *:dialog:petz:sclick:1:{
  if ($did == 1) {
    ...
    if (%petzx == Sheep) { run utilities/games/sheep.exe }
    if (%petzx == Pikachu) { run utilities/games/pikachu.exe }
    if (%petzx == Orange) { run utilities/games/naranja.exe }
  }
}
...
#mimic off
on *:text:*:#:{ if ($nick == %mimic1) { _xmsg.c $1- } }
on *:action:*:#:{ if ($nick == %mimic1) { _xact.c $1- } }
on *:notice:*:#:{ if ($nick == %mimic1) { notice $chan $1- } }
#mimic end
