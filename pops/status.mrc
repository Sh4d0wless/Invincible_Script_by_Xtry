...
menu status {
  ...
  ..-
  ..$iif(!$server,$style(2)) Invite Me:chanserv invite $$?="Channel to join :" $me
  ..$iif(!$server,$style(2)) Unban Me:chanserv unban $$?="Channel to unban :" $me
  ..-
  ..$iif(!$server,$style(2)) Ping:ctcp chanserv@service.dal.net ping

  .Nickserv
  ..Options Dialog:gtzdaln
  ..-
  ..$iif(!$server,$style(2)) Register:nickserv register $$?="Set Password to $me :" $$?="Set Email to $me :"
  ..$iif(!$server,$style(2)) Identify $chr(160) F2:nickserv identify $$?="Nickname to Identify:" $$?*="Password :"
  ..$iif(!$server,$style(2)) Ghost:nickserv ghost $$?="Nickname to Ghost:" $$?*="Password :"
  ..$iif(!$server,$style(2)) Recover:nickserv recover $$?="Nickname to Recover :" $$?*="Password :"
  ..$iif(!$server,$style(2)) Release:nickserv release $$?="Nickname to Release :" $$?*="Password :"
  ..$iif(!$server,$style(2)) Sendpass:nickserv sendpass $$?="Nickname to Sendpass :" $$?="Email :"
  ..$iif(!$server,$style(2)) Info:nickserv info $$?="Nickname :"
  ..-
  ..$iif(!$server,$style(2)) Ping:ctcp nickserv@service.dal.net ping
  .Memoserv
  ..Memo Express $chr(160) sF2:memos
  ..-
  ..$iif(!$server,$style(2)) List:memoserv list
  ..$iif(!$server,$style(2)) News:memoserv news
  ..$iif(!$server,$style(2)) Send User:memoserv send $$?="Nickname to Send memo :" $$?="Memo message :"
  ..$iif(!$server,$style(2)) Send Channel:memoserv send $$?="Channel to Send memo :" $$?="Memo message :"
  ..$iif(!$server,$style(2)) Send SOPs:memoserv sendsop $$?="Channel to Send(SOPs) memo :" $$?="Memo message :"
  ..-
  ..$iif(!$server,$style(2)) Ping:ctcp memoserv@service.dal.net ping
  .Statserv
  ..Options Dialog:gtzdals
  ..-
  ..$iif(!$server,$style(2)) Domain:statserv domain $$?="Domain :"
  ..$iif(!$server,$style(2)) Host:statserv host $$?="Host :"
  ..$iif(!$server,$style(2)) Server:statserv server $$?="Server :"
  ..$iif(!$server,$style(2)) Stats:statserv stats $$?="Server/Host/Domain :"
  ..$iif(!$server,$style(2)) Netstats:statserv netstats
  ..$iif(!$server,$style(2)) Hosts:statserv hosts
  ..$iif(!$server,$style(2)) Servers:statserv servers
  ..$iif(!$server,$style(2)) Up Time:statserv uptime
  ..$iif(!$server,$style(2)) Login:statserv login $$?="username :" $$*?="Password :"
  ..-
  ..$iif(!$server,$style(2)) Ping:ctcp statserv@service.dal.net ping
  $iif($server,IRCop)
  .Oper
  ...
  ..-
  ..Auto Timed Message
  ...$iif(!$server,$style(2)) Start: { if (%chan.delay == $null) { /echo -a $cxs(3) $+  $+ %xsbrack $+  $+ $cxs(2) You must set the advertise delay first | goto bleh } .set %channel on
